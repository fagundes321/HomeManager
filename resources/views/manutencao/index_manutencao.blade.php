<x-layout title="Casa & Carro - HomeManager">
    <div class="container mt-4">
        ...
        <div class="d-flex justify-content-between align-items-center mb-3">
            <h2 class="fw-bold text-dark m-0">Manutenções</h2>
            <div class="d-flex gap-2">
                <a href="{{ route('index') }}" class="btn btn-outline-dark btn-sm">Menu</a>
                <a href="{{ route('locais.index') }}" class="btn btn-outline-dark btn-sm">Locais</a>
                <a href="{{ route('cidades.index') }}" class="btn btn-outline-dark btn-sm">Cidades</a>
                <a href="{{ route('manutencao.create') }}" class="btn btn-dark btn-sm">+ Novo Item</a>
            </div>
        </div>

        @forelse ($locais as $local)
            @php
                $lugaresDasManutencoes = $manutencao->where('local_id', $local->id);
            @endphp

            @if ($lugaresDasManutencoes->count())
                <h3 class="fw-bold text-dark mt-4 mb-2">{{ $local->local }}</h3>
                    ...
                    <div class="card shadow-sm border border-dark mb-4">
                        <div class="table-responsive">
                            <table class="table table-hover align-middle mb-0 text-center">
                                ...
                                <tbody>
                                    @foreach ($lugaresDasManutencoes as $item)
                                        <tr>
                    ...
                    @foreach ($lugaresDasManutencoes as $item)
                        <div class="card shadow-sm mb-3 border-dark">
                            <div class="card-body">
                                ...
                                @if ($item->observacao)
                                    <div class="small text-muted mb-2">{{ $item->observacao }}</div>
                                @endif
        ...
        @empty
            ...
            <div class="card shadow-sm border-dark text-center">
                <div class="card-body py-5">
                    <h6 class="fw-bold mb-2">Nenhuma manutenção cadastrada</h6>
